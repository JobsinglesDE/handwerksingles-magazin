...
/**
 * Betriebsliste fuer eine Gewerk×Stadt-Seite (Server-Komponente, alles im SSR-HTML).
 * Jede Karte verlinkt auf das Betriebs-Profil (Vanity-Such-Einstieg).
...
export function BetriebList({
  ...
  return (
    <ul className="grid grid-cols-1 sm:grid-cols-2 gap-4 list-none p-0">
      {betriebe.map((b) => {
        const href = getBetriebUrl(bundesland, stadt, b.gewerk, b.slug);
        const addr = [b.street, [b.plz, b.city].filter(Boolean).join(' ')].filter(Boolean).join(', ');
        return (
          <li
            key={b.slug}
            className="group relative rounded-2xl bg-surface border border-foreground/10 p-5 ambient-shadow hover-lift hover:border-brand-orange/40 transition-colors"
          >
